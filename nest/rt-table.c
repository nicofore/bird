...
static struct fib_node *getNextNode(struct fib_node *fn){
	return (struct fib_node *) (atomic_load(&(fn->next)) & ~1);
}
...
net_route_ip6_sadr_trie(struct rtable_private *t, const net_addr_ip6_sadr *n0)
{
	TRIE_WALK_TO_ROOT_IP6(t->trie, (const net_addr_ip6 *)n0, px)
	{
		...
		struct fib_node *fn;
		uint row = reserve_row(&(t->fib));

		atomic_uintptr_t *curr = &(t->fib.soft_links[row][0]);
...
net_roa_check_ip4_trie(struct rtable_private *tab, const net_addr_ip4 *px, u32 asn)
{
	...
	TRIE_WALK_TO_ROOT_IP4(tab->trie, px, px0)
	{
		...
		struct fib_node *fn;
		uint row = reserve_row(&(tab->fib));
		atomic_uintptr_t* curr = &(tab->fib.soft_links[row][0]);
		struct fib_node* p = fib_get_chain(&(tab->fib), (net_addr *) (&roa0), row);
		atomic_store(curr, (atomic_uintptr_t) p);
