...
struct fib
{
  ...
  pthread_mutex_t **fib_locks;    /* Locks for hash table entries */
  ...
  atomic_char resize_lock;       /* Lock for rehashing */
};
...
#define FIB_ITERATE_START(f, it, type, z)          \
  do                                                 \
  { \
    struct fib * f_ = (f);                                                 \
    pthread_mutex_lock(&f_->fib_locks[(it)->hash]); \
    struct fib_node *fn_ = fit_get(f_, it);         \
    uint count_ = (f_)->hash_size;                  \
    uint hpos_ = (it)->hash;                         \
    type *z;                                         \
    for (;;)                                         \
    {                                                \
      if (!fn_)                                      \
      {                                              \
        pthread_mutex_unlock(&f_->fib_locks[hpos_]);\
        if (++hpos_ >= count_)                       \
          break;                                     \
...
//False but only use in rip so :(
#define FIB_ITERATE_PUT_END(it) fit_put_end(it) ;

#define FIB_ITERATE_UNLINK(it, fib) pthread_mutex_lock(&(fib)->fib_locks[(it)->hash]); fit_get(fib, it) ;
...
typedef struct rte
{
  ...
  union
  { /* Protocol-dependent data (metrics etc.) */
#ifdef CONFIG_RIP
    struct
    {
...
static inline net *net_find_valid(rtable *tab, const net_addr *addr)
{
  ...
  return (n && rte_is_valid(n->routes)) ? n : NULL;
}
